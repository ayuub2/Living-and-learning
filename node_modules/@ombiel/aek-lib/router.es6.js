...
class Router {
  ...
  addRoute(path,cb) {
    ...
    this.callbacks.push(route.middleware(cb));
  }
  ...
  onHashChange() {
    if(!this._silentHashChange) {
      ...
      path = this.cleanPath(path);
      var historyIndex = this.history.indexOf(path);
      if(this.useHash && window.history.state && history.state.currentHistoryIndex != null) {
        historyIndex = window.history.state.currentHistoryIndex;
      }

      if(historyIndex > -1 && historyIndex < this.currentHistoryIndex) {
        this.backTo(path,false);
      }
      else {
        this.goto(path);
      }
